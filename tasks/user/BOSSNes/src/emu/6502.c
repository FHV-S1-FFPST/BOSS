...
static void bcc() {
	if ((status & FLAG_CARRY) == 0) {
		oldpc = pc;
		pc += reladdr;
		if ((oldpc & 0xFF00) != (pc & 0xFF00)) cycles += 2; //check if jump crossed a page boundary
		else cycles++;
	}
}
...
static void sax() {
	...
	stx();
	putvalue(a & x);
	if (penaltyop && penaltyaddr) cycles--;
}
...
#ifdef DEBUG_DUMP
extern void dasm(unsigned op,
	unsigned PC,
	unsigned A, unsigned X, unsigned Y, unsigned Flags, unsigned SP,
	unsigned PPU_X, unsigned PPU_Scanline);
uint16_t lastop;
#endif

uint64_t exec6502(uint64_t ticks) {
