...
extern int16_t msgbox(uint8_t *msgtxt, uint16_t buttons);
...
bool mapperinit(struct cart_s *cart) {
	...
	switch (cart->mapper) {
		...
		case 1: //MMC1
			mappermemory = malloc(sizeof(struct map1_s));
		...
		case 4: //MMC3
			mappermemory = malloc(sizeof(struct map4_s));
		...
		case 9: //MMC2
		case 10: //MMC4
			mappermemory = malloc(sizeof(struct map9_s));
