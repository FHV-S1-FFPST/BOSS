...
static void drawCharacterWithColor(char c, uint32_t color);
static void drawCharacter(char c);
static void drawHorizontalLine(uint32_t offset, uint32_t length);
static void drawVerticalLine(uint32_t offset, uint32_t length);
static void drawStringWithColor(char *s, uint32_t color);
...
static RETURN_CODE ioctlHDMI(struct driver *self, enum IOCTL_CMD cmd, void *data) {
	...
	case DRAW_HORIZONTAL_LINE:
		ol = (OffsetLengthData_t *)data;
		drawHorizontalLine(ol->offset, ol->length);
		break;

	case DRAW_VERTICAL_LINE:
		ol = (OffsetLengthData_t *)data;
		drawVerticalLine(ol->offset, ol->length);
		break;

	case DRAW_STRING_WITH_COLOR:
		dscd = (DrawStringColorData_t *)data;
		drawStringWithColor(dscd->message, dscd->color);
		break;

	case ATTACH_FRAMEBUFFER:
		framebuffer = (char *) data;
...
Driver_t *createHDMIDriver() {
	Driver_t *hdmiDriver = (Driver_t *)malloc(sizeof(Driver_t));
	hdmiDriver->open = &openHDMI;
	hdmiDriver->write = &writeHDMI;
	hdmiDriver->read = NULL;
	hdmiDriver->ioctl = &ioctlHDMI;
